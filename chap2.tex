...
\end{definition}
\begin{example}\label{program:p1}
	考虑以下的程序$P_1$：
	$$p \rightarrow .$$
	$$p \rightarrow r.$$
	$$q \rightarrow r.$$
...
\end{definition}
\begin{example}\label{program:p2}
	考虑以下的程序$P_2$:
	$$p \vee q \leftarrow r.$$
	$$r \leftarrow p.$$
	$$r \leftarrow q.$$
...
\begin{definition}[$GL$规约]
	给定一个不含约束的正规逻辑程序$P$和原子集合$S$， $P$基于$S$的$GL$规约(Gelfond-Lifschitz reduction)[14]，记为$P^S$，是对$P$做以下操作所得到的程序：
	\begin{enumerate}
...
\begin{definition}[析取逻辑程序的回答集]
	给定一个析取逻辑程序P和原子集合$S$，记$P$去掉约束后的程序为$P'$，$P'$的最小模型的集合为$\Gamma(P^S)$。$S$是$P$的回答集，当且仅当$S \in \Gamma(P^S)$，并且不违反$P$中的约束。
\end{definition}
...
\begin{definition}[补全]
	给定一个逻辑程序$P$，其补全$Comp(P)$是$P$的约束和$P$的克拉克补全(Clark completion)的并集。它包括以下子句：
	\begin{enumerate}
	...
	\end{enumerate}
\end{definition}
\begin{example}\label{program:p3}
	给定程序$P_3$：
...
\end{definition}
\begin{example}
	程序$P_1$和$P_2$的正依赖图是一样的，如图2.1所示。
	\begin{figure}[htb]
		\center
		\includegraphics[width=300pt]{img/dg.png}\\
		\caption{$P_1$和$P_2$的正依赖图}\label{fig:dg}
	\end{figure}
\end{example}
\begin{definition}[环]
...
\begin{example}\label{program:p4}
	考虑如下正规逻辑程序$P_4$：
	$$a \leftarrow b.$$
	$$b \leftarrow a.$$
	$$a \leftarrow not\ c.$$
...
\end{definition}
\begin{definition}[基本环]
	给定环$L$和程序$P$，若$L$的所有非空真子集在$L$里都是向外的，那么我们称$L$是基本环(elementary loop)。
\end{definition}
\begin{example}
	程序$P_2$有5个基本环：$\lbrace p\rbrace ,\lbrace r\rbrace ,\lbrace q\rbrace ,\lbrace p,r\rbrace ,\lbrace q,r\rbrace$。
\end{example}
\begin{theorem}
	给定程序$P$和原子集合$S$，如果$S$满足$P$，那么以下结论与定理\ref{theorem:eq1}的都是等价的：
	\begin{enumerate}
