...
\begin{proof}
	...
	下面，我们将证明，如果存在$P$的环$L'$，满足$L'\subset L$，$R^-_L(L')\subseteq R^-(L)$，或者$head(R^-(L'))\cap (L'\cap L)\subseteq head(R^-(L))\cap L$，$R^-_L(L')\subset R^-(L)$，那么算法\ref{algo_PLxing}可能返回一个原子集合$C'$，$C'\subset L$，也可能返回$L$。
	...
	\begin{enumerate}
		...
		\item 其他情况，此时$R^-_L(C)\nsubseteq R^-(L)$。对于任意的规则$r\in R^-_L(C)\backslash R^-(L)$，若$|head(r)\cap C|=1$，则$r\notin R^-_L(L')$，因为这与$R^-_L(L')\subseteq R^-(L)$矛盾，此时有$L'\cap (head(r)\cap C)= \emptyset$；若$|head(r)\cap C|>1$，则可能会有$r\in R^-(L')$，$head(r)\cap (L\backslash L')\ne \emptyset$，所以有$L'\cap (head(r)\cap C)\ne \emptyset$。因此，如果$|head(r)\cap C|=1$，那么$L'$仍然在$G_P$关于原子集合$C\backslash (head(R_L^-(C)\backslash R^-(L))$的诱导子图的强连通分量中，并且该连通分量会被加入到$SCC$。在之后的迭代，算法\ref{algo_PLxing}会继续分析该强连通分量。而如果$|head(r)\cap C|>1$，那么$L'$可能不在$G_P$关于原子集合$C\backslash head(R_L^-(C)\backslash R^-(L))$的诱导子图的强连通分量中，然而，算法\ref{algo_PLxing}同样把$head(R_L^-(C)\backslash R^-(L))$删除了，在之后的迭代，它只会处理到$G_P$关于原子集合$C\backslash head(R_L^-(C)\backslash R^-(L))$的诱导子图的强连通分量，因此，它可能会错误这个$L'$。
	\end{enumerate}
	
	综合上述，算法\ref{algo_PLxing}如果返回$C$，$C\ne L$，那么$L$不是特征环；如果返回$L$，那么$L$可能是特征环。
\end{proof}

\section{弱基本环和弱特征环}

在上一章和本章上一节，本文分别讨论过析取逻辑程序的基本环和特征环的概念。
针对识别析取逻辑程序的基本环和特征环是coNP-complete这一问题，本节将提出弱基本环和弱特征环的概念，同时，给出多项式时间复杂度的识别算法。

\subsection{弱基本环的定义}

识别析取逻辑程序的基本环是coNP-complete的，但是，我们可以降低基本环的要求，由此提出限制小一点的弱基本环。下面，我们将提出弱特征环概念，并给出识别弱特征环的算法。
...
另一方面，算法\ref{algo_WeakElementaryLoopDLP3}只有一层循环，里面所调用的算法\ref{algo_WeakElementaryLoopDLP}和算法\ref{algo_WeakElementaryLoopDLP2}的时间复杂度分别为$O(n^3 )$和$O(m)$，所以算法\ref{algo_WeakElementaryLoopDLP3}的时间复杂度为$O(mn^3)$，其中，$m$为规则的个数，$n$为原子的个数。
