...
利用算法\ref{algo_WeakElementaryLoopDLP}和算法\ref{algo_WeakElementaryLoopDLP2}，本文在HWEF程序的判断过程中，使用近似处理，那么就可以在多项式时间内判断给定程序是否属于HWEF程序的子类，记该子类为HWEF*。下面，本文给出算法\ref{algo_WeakElementaryLoopDLP3}。
...
\begin{proof}
	...
	下面，我们将证明算法\ref{algo_WeakElementaryLoopDLP3}的时间复杂度。算法\ref{algo_WeakElementaryLoopDLP3}包括两部分的操作：生成$S$和验证$S$中的元素。其中，最坏情况下，生成$S$的时间代价$T_1=n^2$，其中$n$为原子数目。注意到，$|S|=T_1=n^2$。因此，验证$S$中的元素的时间代价$T_2=|S|\times (|E|^3+nm)=n^2\times (2^3+nm)$，其中$m$为规则数目。所以，算法\ref{algo_WeakElementaryLoopDLP3}的时间代价为$T=T_1+T_2=n^2\times (9+nm)$，即$O(mn^3)$。
\end{proof}
