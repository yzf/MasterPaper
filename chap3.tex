...
虽然准确地识别析取逻辑程序的基本环的时间代价太大，但是，如果可以容忍一定的出错率，在识别过程中，可以使用近似处理，那么就能在多项式时间内判断一个环$L$是否属于基本环的一个超集，记为$EL^*(P)$。

下面，我们给出算法\ref{algo_ElementaryLoopXing}。算法\ref{algo_ElementaryLoopXing}的过程和算法\ref{algo_ElementaryLoop}相似，通过自顶向下的策略进行广度优先搜索，仅考虑环$L$的子环中所有值得检测的部分。对于每一个子环$C$，有两种情况：

\begin{enumerate}
	\item 如果$R_L^-(C) \subseteq R^-(L)$，那么根据定义\ref{chap3_definition_el_dlp}，环$L$不是基本环；
	\item 如果$R_L^-(C) \nsubseteq R^-(L)$，那么对于任意规则$r \in R_L^-(C) \backslash R^-(L)$：
	\begin{enumerate}
		\item 若$|head(r) \cap C|=1$，则$head(r) \cap C$不可能在非向外的环$L'$里，其中，$L' \subset C$。否则，就会有$r \in R_L^-(L')$，而由$L'$是非向外的，有$R_L^-(L') \subseteq R^-(L)$，从而得出$r \in R_L^-(L)$，矛盾。因此，此时可以删除$head(r)$；
		\item 若$|head(r) \cap C|>1$，则可能会有$L' \subseteq C$，使得$r \in R^-(L')$，$head(r) \cap (L \backslash L') \neq \emptyset$。此时，$L' \cap (head(r) \cap C) \neq \emptyset$，根据基本环的定义，不能删除$head(r)$。然而，为了优化识别时间，该算法采取了近似操作，在这里同样对$head(r)$进行删除。
	\end{enumerate}
\end{enumerate}
